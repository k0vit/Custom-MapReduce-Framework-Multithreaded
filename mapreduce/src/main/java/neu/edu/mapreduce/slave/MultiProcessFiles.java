...
public class MultiProcessFiles implements Runnable {
	...
	private Mapper<?, ?, ?, ?> instantiateMapper() {
		...
		try {
			...
			if (mapper != null) {
				log.info(String.format("[Thread %d] Mapper instantiated successfully %s", threadNo, jobConfiguration.getProperty(MAPPER_CLASS)));
			}
		} catch (InstantiationException | IllegalAccessException e) {
			log.severe(String.format("[Thread %d] Failed to create an instance of mapper class %s. Reason: %s", threadNo, mapper, e.getMessage()));
	...
	private void processLine(String line, Mapper<?, ?, ?, ?> mapper, Context context, long counter) {
		try {
			...
			log.severe(String.format("[Thread %d] Failed to invoke map method on mapper class %s. Reason: %s", threadNo, mapper, e.getMessage()));
			log.severe(String.format("[Thread %d] Stacktrace %s", threadNo, Utilities.printStackTrace(e)));
		}
	}
	...
	private Class<?> getMapreduceClass(String className) {
		...
		try {
			...
			log.severe(String.format("[Thread %d] Failed to find class %s. Reason: %s", threadNo, className, e.getMessage()));
			log.severe(String.format("[Thread %d] Stacktrace %s", threadNo, Utilities.printStackTrace(e)));
		}
		return mrClass;
	}
